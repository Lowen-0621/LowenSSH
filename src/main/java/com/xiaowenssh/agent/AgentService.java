...
import com.xiaowenssh.agent.guard.ConfirmationHandler;
import com.xiaowenssh.persistence.AuditService;
import com.xiaowenssh.persistence.MessageService;
...
public class AgentService {
    ...
    private static final String SYSTEM_PROMPT = """
            你是 XiaowenSSH，一个 AI 运维助手，可以通过工具在目标 Linux 服务器上执行命令、读文件、看日志。
    ...
    public String run(Long sessionId, String task, SshTools tools, ConfirmationHandler confirmer) {
        ...
        for (int round = 1; round <= MAX_ROUNDS; round++) {
            ...
            if (!response.hasToolCalls()) {
                ...
                if (text == null || text.isBlank()) {
                    text = "任务结束，但模型未给出文字结论（可能命令被安全策略阻止，无法完成原请求）。";
        ...
        return "已达到最大循环轮数（" + MAX_ROUNDS + "），任务可能未完成。请拆分任务后重试。";
    }
        ...
        Thread worker = new Thread(() -> {
            try {
                ...
                for (int round = 1; round <= MAX_ROUNDS; round++) {
                    ...
                    ChatResponse response = aggregatedRef.get();
                ...
                sink.tryEmitNext(new AgentEvent.Done(
                        "已达到最大循环轮数（" + MAX_ROUNDS + "），任务可能未完成。请拆分任务后重试。"));
                sink.tryEmitComplete();
            } catch (Exception e) {
    ...
    private void emitToolResults(ToolExecutionResult execResult, Sinks.Many<AgentEvent> sink) {
        ...
        if (last instanceof ToolResponseMessage trm) {
            for (ToolResponseMessage.ToolResponse resp : trm.getResponses()) {
                ...
                String summary = data == null ? "" : (data.length() > 500 ? data.substring(0, 500) + "…" : data);
    ...
    private void persistToolResponses(Long sessionId, List<ToolResponseMessage.ToolResponse> responses) {
        for (ToolResponseMessage.ToolResponse resp : responses) {
            messageService.saveToolResult(sessionId, resp.id(), resp.responseData());
        }
    }
