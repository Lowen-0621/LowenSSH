...
class _KeysBody extends ConsumerWidget {
          ...
          child: keys.isEmpty
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (ctx, i) {
                    final k = keys[i];
                    // 统计引用该密钥的主机数
                    final usedBy = hosts.where((h) => h.keyId == k.id).length;
  ...
  Widget _keyRow(BuildContext context, WidgetRef ref, SshKey k, int usedBy) {
    final l = ref.watch(l10nProvider);
    return Container(
                    ...
                    FilledButton(
                      ...
                      onPressed: () {
                        ...
                        if (!pemText.contains('-----BEGIN')) {
                          setState(() => errorText = l.t('keys.errPem'));
                          return;
                        }
