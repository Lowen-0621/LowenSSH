...
/// 应用级通用设置
class AppSettings {
  static const int currentUiStyleVersion = 2;

  final AppLang lang;
  final int uiStyleVersion;
  ...
  const AppSettings({
    ...
    this.uiStyleVersion = currentUiStyleVersion,
    this.themeId = 'pearl',
  ...
  factory AppSettings.fromJson(Map<String, dynamic> j) {
    final storedUiVersion = j['uiStyleVersion'] as int? ?? 1;
    return AppSettings(
      lang: (j['lang'] as String?) == 'en' ? AppLang.en : AppLang.zh,
      uiStyleVersion: currentUiStyleVersion,
      // 新视觉首次上线时迁移旧配色；之后仍尊重用户主动选择的主题。
      themeId: storedUiVersion < currentUiStyleVersion
          ? 'pearl'
          : j['themeId'] as String? ?? 'pearl',
  ...
  AppSettings copyWith({
    ...
    lang: lang ?? this.lang,
    uiStyleVersion: uiStyleVersion ?? this.uiStyleVersion,
